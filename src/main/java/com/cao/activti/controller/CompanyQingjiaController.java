...
@RestController
@RequestMapping("/companyQingjia")
public class CompanyQingjiaController {

    @Autowired
    private CompanyQingjiaService qingjiaService;
    @Autowired
    private ImageService imageService;

    // 请假申请并开启流程
    @PostMapping(value = "/insertCompanyQingjia")
    public AjaxObj insertCompanyQingjia(Qingjia qingjia){
        qingjiaService.insertCompanyQingjia(qingjia);
        return  new AjaxObj(ReturnValCode.RTN_VAL_CODE_SUCCESS,"新增成功");
    }

    // 组长或部门经理审批
    @GetMapping(value = "/agree")
    public AjaxObj agree(String taskId,String uuid,String userId){
        qingjiaService.agree(taskId,uuid,userId);
        return  new AjaxObj(ReturnValCode.RTN_VAL_CODE_SUCCESS,"请求成功");
    }

    // 总经理审批或直接结束
    @GetMapping(value = "/endOrManager")
    public AjaxObj endOrManager(String taskId,String status){
        qingjiaService.endOrManager(taskId);
        return  new AjaxObj(ReturnValCode.RTN_VAL_CODE_SUCCESS,"请求成功");
    }
